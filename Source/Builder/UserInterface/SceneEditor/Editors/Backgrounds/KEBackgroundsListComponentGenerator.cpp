...
/*! \class osg::BackgroundsListComponentGenerator
A UI Backgrounds List ComponentGenerator. 	
*/
...
void BackgroundsListComponentGenerator::initMethod (void)
{
}
...
ComponentPtr BackgroundsListComponentGenerator::getListComponent(::osg::ListPtr Parent, const boost::any& Value, ::osg::UInt32 Index, bool IsSelected, bool HasFocus)
{
	if(Value.empty() ||
		Value.type() != typeid(BackgroundPtr)){
			return Panel::createEmpty();
	}

    BackgroundPtr TheBackground;
    try
    {
        TheBackground = boost::any_cast<BackgroundPtr>(Value);
    }
	...
	if(TheLabelComponent->getType().isDerivedFrom(TextComponent::getClassType()))
	{
		...
		if(getName(TheBackground) != NULL)
		{
			ValueString = getName(TheBackground);
		}
		else
		{
			ValueString = TheBackground->getType().getCName();
		}
	...
	{
		TheIconComponent = _IconManager->createIconImageComponent(TheBackground->getType().getCName());
	}
	if(TheIconComponent ==  NullFC)
	{
		TheIconComponent = _IconManager->createIconImageComponent("UnknownBackground");
	}
	if(TheIconComponent ==  NullFC)
	{
	...
	beginEditCP(TheComponent, Panel::ChildrenFieldMask | Panel::LayoutFieldMask);
		...
		TheComponent->setPreferredSize(Vec2f(150.0f,20.0f));
...
BackgroundsListComponentGenerator::BackgroundsListComponentGenerator(void) :
    Inherited(),
		_IconManager(NULL)
{
}

BackgroundsListComponentGenerator::BackgroundsListComponentGenerator(const BackgroundsListComponentGenerator &source) :
    Inherited(source),
...
void BackgroundsListComponentGenerator::changed(BitVector whichField, ::osg::UInt32 origin)
{
    Inherited::changed(whichField, origin);
}

void BackgroundsListComponentGenerator::dump(      ::osg::UInt32    , 
                         const BitVector ) const
{
    SLOG << "Dump BackgroundsListComponentGenerator NI" << std::endl;
}
